...
function sendCodeClipBindingAuditUnavailable(res) {
  return res.status(503).json({
    ok: false,
...
function sendCodeClipBindingAuditInvalid(res) {
  return res.status(400).json({
    ok: false,
...
function sendCodeClipBindingOperatorContractError(
  res,
  status,
  code,
  message,
...
function providerBindingConflictDetails(error = {}) {
  const details = error.details || {};
  const safeDetails = {};
  for (const key of ["bindingId", "eventCode", "provider", "channel", "status", "reactivationRequired"]) {
...
function isProviderBindingAuditRequestError(error) {
  return (
    error?.name === "CodeClipProviderAccountBindingError" &&
    [
      ...
      "INVALID_PROVIDER_BINDING_AUDIT_QUERY",
      "INVALID_PROVIDER_BINDING_AUDIT_CURSOR",
    ].includes(error.code)
  );
}

function cloneCodeClipAuditState(state) {
  if (state === undefined || state === null) return null;
  if (typeof structuredClone === "function") return structuredClone(state);
  return JSON.parse(JSON.stringify(state));
}

function isPlainCodeClipBindingUpdateBody(body) {
  if (!body || typeof body !== "object" || Array.isArray(body)) return false;
  const prototype = Object.getPrototypeOf(body);
  return prototype === Object.prototype || prototype === null;
}

function hasOnlyAllowedCodeClipBindingUpdateFields(body = {}) {
...
app.post("/internal/codeclip/events/:eventCode/provider-bindings", requireCodeClipAdmin, async (req, res) => {
  ...
  try {
    ...
    const result = await withCodeClipCorePersistenceTransaction(async ({ queryClient }) => {
      const created = await createCodeClipProviderAccountBinding(
        {
          provider: req.body?.provider,
        ...
        },
        {
          ...
          getEventByCode: async (candidateEventCode) =>
            String(candidateEventCode || "").trim() === eventCode ? event : null,
        }
      );
        ...
        await appendCodeClipProviderAccountBindingAuditEvent(
          {
            ...
            actorId: null,
            beforeState: null,
            afterState: cloneCodeClipAuditState(toAuditState(created.row)),
          },
          { queryClient }
        );
      }
...
app.get("/internal/codeclip/provider-bindings", requireCodeClipAdmin, async (req, res) => {
  const {
  ...
  try {
    const result = await listCodeClipProviderAccountBindings(
      {
    ...
    return res.json({
      ok: true,
      items: result.items.map(toPublicCodeClipProviderBinding),
      page: result.page,
      filters: result.filters,
    });
  } catch (error) {
    if (isProviderBindingError(error, "INVALID_PROVIDER_BINDING")) {
      const code = error.details?.reason === "INVALID_CURSOR" ? "INVALID_CURSOR" : "VALIDATION_ERROR";
      return sendCodeClipBindingOperatorContractError(
        res,
        400,
        code,
        code === "INVALID_CURSOR" ? "Invalid cursor." : "Invalid provider binding request."
      );
    }
    console.warn("codeClip provider binding operator global list failed", {
      vertical: "codeclip",
      route: "/internal/codeclip/provider-bindings",
      operationalEvent: "provider_binding_global_list_failed",
    });
    return sendCodeClipBindingOperatorContractError(
      res,
      500,
      "INTERNAL_ERROR",
      "Provider binding operation unavailable."
    );
  }
});

app.post("/internal/codeclip/provider-bindings", requireCodeClipAdmin, async (req, res) => {
  const {
    createCodeClipProviderAccountBinding,
  ...
  try {
    if (req.body?.vertical !== undefined && typeof req.body.vertical !== "string") {
      return sendCodeClipBindingOperatorContractError(
        res,
        400,
        "VALIDATION_ERROR",
        "Invalid provider binding request."
      );
    }
    const requestedVertical = String(req.body?.vertical || "codeclip").trim().toLowerCase();
    if (requestedVertical !== "codeclip") {
      return sendCodeClipBindingOperatorContractError(
        res,
        400,
    ...
    let eventCode;
    try {
      eventCode = normalizeCodeClipProviderBindingEventCode(req.body?.eventCode ?? req.body?.event_code);
    } catch {
      return sendCodeClipBindingOperatorContractError(
        res,
        400,
    ...
    if (!result.created) {
      return sendCodeClipBindingOperatorContractError(
        ...
        {
          bindingId: result.row?.id,
          eventCode: result.row?.eventCode,
    ...
    if (isProviderBindingError(error, "PROVIDER_ACCOUNT_BINDING_CONFLICT")) {
      return sendCodeClipBindingOperatorContractError(
        res,
        409,
        error.details?.reactivationRequired
          ? "REACTIVATION_REQUIRED"
          : "PROVIDER_ACCOUNT_ALREADY_BOUND",
        error.details?.reactivationRequired
          ? "This provider account has a disabled binding that must be reactivated."
          : "This provider account is already connected.",
        providerBindingConflictDetails(error)
      );
    }
    console.warn("codeClip provider binding operator global create failed", {
      vertical: "codeclip",
      route: "/internal/codeclip/provider-bindings",
      operationalEvent: "provider_binding_global_create_failed",
    });
    return sendCodeClipBindingOperatorContractError(
      res,
      500,
...
app.get("/internal/codeclip/provider-bindings/:bindingId/audit", requireCodeClipAdmin, async (req, res) => {
  ...
  try {
    const normalizedBindingId = normalizeCodeClipProviderBindingId(bindingId);
    const binding = await getCodeClipProviderAccountBindingById(normalizedBindingId, {
      queryClient: database.pool,
    });

    if (!binding) return sendCodeClipBindingOperatorNotFound(res);

    const pageResult = await listCodeClipProviderAccountBindingAuditEventsPage(
      { bindingId: normalizedBindingId, limit: req.query?.limit, cursor: req.query?.cursor },
      { queryClient: database.pool }
    );
    const items = pageResult.items.map(toPublicCodeClipProviderBindingAuditEvent).filter(Boolean);
    ...
    if (isProviderBindingAuditRequestError(error)) {
      return sendCodeClipBindingAuditInvalid(res);
    }
    console.warn("codeClip provider binding audit read failed", {
      vertical: "codeclip",
      route: "/internal/codeclip/provider-bindings/:bindingId/audit",
      operationalEvent: "provider_binding_audit_read_failed",
    });
    return sendCodeClipBindingAuditUnavailable(res);
  }
});

app.get("/internal/codeclip/provider-bindings/:bindingId", requireCodeClipAdmin, async (req, res) => {
  const bindingId = String(req.params.bindingId || "").trim();
  const {
    getCodeClipProviderAccountBindingById,
    normalizeCodeClipProviderBindingId,
  ...
  try {
    ...
    console.warn("codeClip provider binding operator read failed", {
      vertical: "codeclip",
      route: "/internal/codeclip/provider-bindings/:bindingId",
      operationalEvent: "provider_binding_read_failed",
    });
    return sendCodeClipBindingOperatorUnavailable(res);
  }
});

app.patch("/internal/codeclip/provider-bindings/:bindingId", requireCodeClipAdmin, async (req, res) => {
  const bindingId = String(req.params.bindingId || "").trim();
  const {
    getCodeClipProviderAccountBindingById,
  ...
  try {
    const normalizedBindingId = normalizeCodeClipProviderBindingId(bindingId);
    if (
      !isPlainCodeClipBindingUpdateBody(req.body) ||
      !hasOnlyAllowedCodeClipBindingUpdateFields(req.body)
    ) {
      return sendCodeClipBindingOperatorContractError(
        res,
        400,
    ...
    const result = await withCodeClipCorePersistenceTransaction(async ({ queryClient }) => {
      const existing = await getCodeClipProviderAccountBindingById(normalizedBindingId, { queryClient });
      if (!existing) return { updated: false, row: null };

      const normalizedDisplayName = normalizeCodeClipProviderBindingDisplayName(
      ...
      if (!updated) return { updated: false, row: null };
      ...
      await appendCodeClipProviderAccountBindingAuditEvent(
        {
          ...
          actorType: "operator_key",
          actorId: null,
          beforeState: cloneCodeClipAuditState(toAuditState(existing)),
          afterState: cloneCodeClipAuditState(toAuditState(updated)),
        },
        { queryClient }
      );

      return { updated: true, row: updated };
    });

    if (!result.row) return sendCodeClipBindingOperatorNotFound(res);
    ...
    console.warn("codeClip provider binding operator update failed", {
      vertical: "codeclip",
      route: "/internal/codeclip/provider-bindings/:bindingId",
      operationalEvent: "provider_binding_update_failed",
    });
    return sendCodeClipBindingOperatorUnavailable(res);
  }
});
...
app.post("/internal/codeclip/provider-bindings/:bindingId/disable", requireCodeClipAdmin, async (req, res) => {
  ...
  try {
    ...
    const result = await withCodeClipCorePersistenceTransaction(async ({ queryClient }) => {
      const existing = await getCodeClipProviderAccountBindingById(normalizedBindingId, { queryClient });
      if (!existing) return { disabled: false, row: null };
      if (existing.status === "disabled") return { disabled: false, row: existing };

      const disabled = await disableCodeClipProviderAccountBinding(normalizedBindingId, { queryClient });
      if (!disabled) return { disabled: false, row: null };
      ...
      await appendCodeClipProviderAccountBindingAuditEvent(
        {
          ...
          actorId: null,
          beforeState: cloneCodeClipAuditState(toAuditState(existing)),
          afterState: cloneCodeClipAuditState(toAuditState(disabled)),
        },
        { queryClient }
      );

      return { disabled: true, row: disabled };
    });
...
app.post("/internal/codeclip/provider-bindings/:bindingId/reactivate", requireCodeClipAdmin, async (req, res) => {
  const bindingId = String(req.params.bindingId || "").trim();
  const {
    getCodeClipProviderAccountBindingById,
    normalizeCodeClipProviderBindingId,
  ...
  try {
    ...
    const result = await withCodeClipCorePersistenceTransaction(async ({ queryClient }) => {
      ...
      if (existing.status === "active") return { reactivated: false, row: existing };

      const reactivated = await reactivateCodeClipProviderAccountBinding(normalizedBindingId, {
        queryClient,
      });
      if (!reactivated.row) return reactivated;
        ...
        await appendCodeClipProviderAccountBindingAuditEvent(
          {
            ...
            actorId: null,
            beforeState: cloneCodeClipAuditState(toAuditState(existing)),
            afterState: cloneCodeClipAuditState(toAuditState(reactivated.row)),
          },
          { queryClient }
        );
      }
    ...
    console.warn("codeClip provider binding operator reactivate failed", {
      vertical: "codeclip",
      route: "/internal/codeclip/provider-bindings/:bindingId/reactivate",
      operationalEvent: "provider_binding_reactivate_failed",
    });
    return sendCodeClipBindingOperatorUnavailable(res);
  }
});

app.get("/internal/codeclip/events/:eventCode/provider-binding-audit", requireCodeClipAdmin, async (req, res) => {
  const eventCode = String(req.params.eventCode || "").trim();
  const {
    listCodeClipProviderAccountBindingAuditEventsPage,
    toPublicCodeClipProviderBindingAuditEvent,
  ...
  try {
    ...
    const pageResult = await listCodeClipProviderAccountBindingAuditEventsPage(
      { eventCode, limit: req.query?.limit, cursor: req.query?.cursor },
      { queryClient: database.pool }
    );
    const items = pageResult.items.map(toPublicCodeClipProviderBindingAuditEvent).filter(Boolean);
    ...
    console.warn("codeClip provider binding event audit read failed", {
      vertical: "codeclip",
      route: "/internal/codeclip/events/:eventCode/provider-binding-audit",
      operationalEvent: "provider_binding_event_audit_read_failed",
    });
    return sendCodeClipBindingAuditUnavailable(res);
  }
});
...
function mapCodeClipYouTubeWebSubOperationStatus(code) {
  ...
  if (code === "episode_not_found" || code === "binding_not_found" || code === "subscription_not_found") {
    return 404;
  }
  if (
    code === "episode_not_eligible" ||
    code === "binding_episode_mismatch" ||
    code === "subscription_state_conflict"
  ) {
    return 409;
  }
  if (code === "authentication_unavailable" || code === "public_base_url_unavailable") {
    return 503;
  }
  if (code === "hub_request_timeout" || code === "hub_request_failed") {
    return 502;
  }
  return 500;
}

function mapCodeClipYouTubeWebSubOperationSuccessStatus(result) {
  if (result?.code === "subscription_already_exists") return 200;
  return 202;
}

function sendCodeClipYouTubeWebSubOperationError(res, error) {
  const code = error?.code || "internal_error";
  return res
    .status(mapCodeClipYouTubeWebSubOperationStatus(code))
    .set("Cache-Control", "no-store")
    .json({
      ok: false,
...
app.post("/internal/codeclip/youtube-websub/subscriptions", requireCodeClipAdmin, async (req, res) => {
  ...
  try {
    const result = await createCodeClipYouTubeWebSubSubscriptionOperation(req.body || {}, {
      queryClient: database.pool,
    });
    return res
    ...
    console.warn("codeClip YouTube WebSub subscription create failed", {
      vertical: "codeclip",
      route: "/internal/codeclip/youtube-websub/subscriptions",
      operationalEvent: "youtube_websub_subscription_create_failed",
      error: error?.name || "Error",
    });
...
app.get("/internal/codeclip/youtube-websub/subscriptions", requireCodeClipAdmin, async (req, res) => {
  ...
  try {
    const items = await listCodeClipYouTubeWebSubSubscriptionStatuses(
      {
        providerAccountId: req.query?.providerAccountId,
        status: req.query?.status,
      },
    ...
    console.warn("codeClip YouTube WebSub subscription list failed", {
      vertical: "codeclip",
      route: "/internal/codeclip/youtube-websub/subscriptions",
...
app.get("/internal/codeclip/youtube-websub/subscriptions/:callbackId", requireCodeClipAdmin, async (req, res) => {
  const {
    CodeClipYouTubeWebSubOperationError,
  ...
  try {
    const subscription = await getCodeClipYouTubeWebSubSubscriptionStatus(
      req.params.callbackId,
      { queryClient: database.pool }
    );
    if (!subscription) {
      return sendCodeClipYouTubeWebSubOperationError(res, { code: "subscription_not_found" });
    }
    return res
      .set("Cache-Control", "no-store")
      .json({ ok: true, subscription });
  } catch (error) {
    if (error instanceof CodeClipYouTubeWebSubOperationError) {
      return sendCodeClipYouTubeWebSubOperationError(res, error);
    }
    console.warn("codeClip YouTube WebSub subscription read failed", {
      vertical: "codeclip",
      route: "/internal/codeclip/youtube-websub/subscriptions/:callbackId",
      operationalEvent: "youtube_websub_subscription_read_failed",
      error: error?.name || "Error",
    });
    return sendCodeClipYouTubeWebSubOperationError(res, { code: "internal_error" });
  }
...
app.post("/internal/codeclip/youtube-websub/subscriptions/:callbackId/renew", requireCodeClipAdmin, async (req, res) => {
  const {
    CodeClipYouTubeWebSubOperationError,
  ...
  try {
    ...
    console.warn("codeClip YouTube WebSub subscription renew failed", {
      vertical: "codeclip",
      route: "/internal/codeclip/youtube-websub/subscriptions/:callbackId/renew",
      operationalEvent: "youtube_websub_subscription_renew_failed",
      error: error?.name || "Error",
    });
    return sendCodeClipYouTubeWebSubOperationError(res, { code: "internal_error" });
  }
...
app.post("/internal/codeclip/youtube-websub/subscriptions/:callbackId/unsubscribe", requireCodeClipAdmin, async (req, res) => {
  const {
    CodeClipYouTubeWebSubOperationError,
  ...
  try {
    ...
    console.warn("codeClip YouTube WebSub subscription unsubscribe failed", {
      vertical: "codeclip",
      route: "/internal/codeclip/youtube-websub/subscriptions/:callbackId/unsubscribe",
      operationalEvent: "youtube_websub_subscription_unsubscribe_failed",
      error: error?.name || "Error",
    });
    return sendCodeClipYouTubeWebSubOperationError(res, { code: "internal_error" });
  }
